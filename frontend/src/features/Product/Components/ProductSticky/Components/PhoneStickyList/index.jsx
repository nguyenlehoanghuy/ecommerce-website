...
function PhoneStickyList(props) {
    ...
    useEffect(() => {
        const fectProduct = async () => {
            try {
                // fetch API
                const result = await categoryAPI.getAll({ category_name: 'Tai nghe' });
    ...
    return (
        <Box sx={{ my: '5px', backgroundColor: 'white', padding: '15px' }}>
            <Paper elevation={0} sx={{ marginBottom: '20px' }}>
                <Typography variant="h6">Tai nghe</Typography>
            </Paper>
            <Paper elevation={0}>
...
export default PhoneStickyList;
